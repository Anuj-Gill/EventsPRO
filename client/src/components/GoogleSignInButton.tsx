...
interface GoogleSignInButtonProps {
  onSuccess?: () => void; // Optional callback after successful sign-in
}
...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({ onSuccess }) => {
  ...
  const [loggedIn, setLoggedIn] = useRecoilState(loggedinStatusState)
  ...
  return (
    <button
      ...
      className="w-full px-6 py-4 bg-white text-gray-900 rounded-xl hover:bg-gray-100 transition-colors flex items-center justify-center space-x-3 relative"
    >
      <svg
      ...
      <span className="font-medium z-10">Continue with Google</span>
    </button>
  );
};
